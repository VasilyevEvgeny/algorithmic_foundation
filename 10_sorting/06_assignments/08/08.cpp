...
private:
    ...
    void Merge(size left, size middle, size right) {
        ...
        if (VERBOSE) {
            std::cout << "=======\nleft = " << left << ", middle = " << middle << ", right = " << right << std::endl;
        }
        ...
        for (size l = left, r = middle + 1; l <= middle || r <= right;) {
            if (r > right || (arr_[l] < arr_[r] && l <= middle)) {
                ...
                ++l;
                if (VERBOSE) {
                    std::cout << "left. buf:" << buf << std::endl;
                }
            }
            else {
                if (arr_[l] > arr_[r] && l <= middle && r <= right) {
                    n_inv_ += middle - l + 1;
                    if (VERBOSE) {
                        std::cout << "n_inv_ -> " << n_inv_ << std::endl;
                    }
                }
                ...
                ++r;
                if (VERBOSE) {
                    std::cout << "right. buf:" << buf << std::endl;
                }
            }
        }
