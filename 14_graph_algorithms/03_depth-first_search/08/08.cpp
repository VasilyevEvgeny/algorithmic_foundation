...
public:
    ...
    void solve() {
        ...
        if (n_components == 1) { std::cout << "YES" << std::endl; }
...
private:
    ...
    void dfs(size node, size prev = -1) {
        ...
        for (const auto& vertex : adjacency_list_[node]) {
            if (!used_[vertex]) {
                if (VERBOSE) { std::cout << ".....!used_[" << vertex << "], vertex = " << vertex <<
                ", call dfs(" << vertex << ")" << std::endl; }
                dfs(vertex, node);
            }
            else if (vertex != prev) {
                if (VERBOSE) { std::cout << ".....vertex != prev" << std::endl; }
                std::cout << "NO" << std::endl;
                exit(0);
            }

            if (VERBOSE) { std::cout << ".....else branch nothing" << std::endl; }
        }
    }
